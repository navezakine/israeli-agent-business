...
11. **Revenue analytics** — A dedicated dashboard view of estimated revenue captured: a headline total (plus how much of it Replai *rescued*), a by-month bar chart, a by-treatment breakdown, and a by-source split that tags each booking as direct / recovered-lead / waitlist-fill. Shows the clinic exactly where the money came from. (Estimated from a configurable average ticket value, not actual payments.)
12. **Business-hours & Shabbat awareness** — Replai knows the clinic's hours (set per day in Settings, including שבת) and acts on them. In chat it still answers 24/7 — its edge — but when the clinic is closed or it's Shabbat, it says so warmly (e.g. "שבת שלום") and only offers times when the clinic is actually open. Just as important: it never sends proactive WhatsApps (reminders, review requests, waitlist offers, lead follow-ups) on Shabbat or outside business hours, so patients are never pinged Saturday morning.
13. **Editable message templates** — The clinic controls the exact wording of every automatic message (advance reminder, day-of reminder, review request, waitlist offer, and the two lead follow-ups) from Settings, with auto-filled placeholders like `{greeting}`, `{time}`, `{day}`, `{treatment}` and a live preview. Each ships with a sensible Hebrew default, so it works out of the box and can be personalized to the clinic's voice. (This is also the structure WhatsApp Business API approved templates require.)
14. **First-time onboarding tour** — The first time a new clinic opens the dashboard, a friendly guided walkthrough starts automatically: it spotlights each section (overview, revenue, patients, leads, reactivation, calendar, controls, settings) one step at a time with a short explanation, like the product tours big apps use. They can skip it, step back and forth, and replay it anytime from a floating button. Gets new clients comfortable in under a minute.
